...
```js
{
  ...
  "gulp-cjsx": "^3.0.0", \\ wrapper para criar task
                          \\ do gulp usando o coffee-react
}
```
    ...
    render: ->
      <div className="child-component">
        ...
        <p>
          ...
          # can only to use ternary comparison if externalized
          ite{ReactHelpers.pluralize(@props.quantity)}
        </p>
...
ReactComponents.ParentComponent = React.createClass {
  ...
  # internal changed function that calls
  # the property bound on external framework
  changed: ->
    @props.changed?(@state)
    return

  # function that updates the internal
  # state and calls @changed
  updateValue: ->
...
# wrap classes with factory to use as
# Function with React.render(Component(), element)
...
.controller "TestDirectiveCtrl", ($scope, $element) ->
  ...
  # function passed to React Component to be
  # called when something changed internally by the component
  $scope.changed = (state)->
